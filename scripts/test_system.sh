...
echo -n "[测试] Metrics 含 data_scope ... "
...
echo -n "[测试] static_rule: temperature_report → local ... "
curl -s -X POST "$EDGE_URL/api/strategy/static_rule" > /dev/null
R=$(curl -s -X POST "$EDGE_URL/api/tasks/submit" -H "Content-Type: application/json" \
...
echo -n "[测试] static_rule: image_detection → edge ... "
curl -s -X POST "$EDGE_URL/api/strategy/static_rule" > /dev/null
R=$(curl -s -X POST "$EDGE_URL/api/tasks/submit" -H "Content-Type: application/json" \
...
echo -n "[测试] Metrics 含 qos_satisfaction_rate ... "
curl -s "$EDGE_URL/api/metrics?scope=recent_100" | grep -q "qos_satisfaction_rate" \
  && echo "PASS" && PASS=$((PASS+1)) || { echo "FAIL"; FAIL=$((FAIL+1)); }

echo -n "[测试] LATE-Offload: smoke_alert reason ... "
curl -s -X POST "$EDGE_URL/api/scenario/normal" > /dev/null
curl -s -X POST "$EDGE_URL/api/strategy/dynamic" > /dev/null
R=$(curl -s -X POST "$EDGE_URL/api/tasks/submit" -H "Content-Type: application/json" \
...
echo -n "[测试] LATE-Offload: temperature_report decision ... "
R=$(curl -s -X POST "$EDGE_URL/api/tasks/submit" -H "Content-Type: application/json" \
...
for f in presentation/ppt_outline.md presentation/demo_script.md presentation/qa.md \
  ...
  [ -f "$PROJECT_ROOT/$f" ] && echo "[测试] $f 存在 ... PASS" && PASS=$((PASS+1)) || { echo "[测试] $f 存在 ... FAIL"; FAIL=$((FAIL+1)); }
done

echo -n "[测试] Frontend :3000 ... "
FE=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:3000" 2>/dev/null || echo "000")
[ "$FE" = "200" ] && echo "PASS" && PASS=$((PASS+1)) || echo "SKIP ($FE)"

echo -n "[测试] GET /api/datasets ... "
# 重启服务以确保 Edge 进程加载 scikit-learn（LATE-Learn）
...
echo -n "[测试] POST /api/datasets/download (MEC+EUA) ... "
curl -s -X POST "$EDGE_URL/api/datasets/download" | grep -q "mec_edge" \
  && echo "PASS" && PASS=$((PASS+1)) || { echo "FAIL"; FAIL=$((FAIL+1)); }

echo -n "[测试] google_cluster manual_only ... "
...
echo -n "[测试] train_late_learn.py oracle (CPU) ... "
...
echo -n "[测试] ablation_late_learn.py 小样本 ... "
...
echo -n "[测试] export_report Oracle/Regret 章节 ... "
python "$PROJECT_ROOT/scripts/export_report.py" > /dev/null 2>&1 \
...
[ -f "$PROJECT_ROOT/docs/datasets.md" ] && echo "[测试] docs/datasets.md 存在 ... PASS" && PASS=$((PASS+1)) || { echo "[测试] docs/datasets.md 存在 ... FAIL"; FAIL=$((FAIL+1)); }

echo -n "[测试] train_late_rl.sh 小样本 ... "
...
echo -n "[测试] GET /api/digital_twin/status ... "
...
echo -n "[测试] export_report LATE-RL ... "
python "$PROJECT_ROOT/scripts/export_report.py" > /dev/null 2>&1 \
...
echo -n "[测试] RL candidate sweep 脚本 ... "
